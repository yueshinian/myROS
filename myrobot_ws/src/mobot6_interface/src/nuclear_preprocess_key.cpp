...
#include "mobot6_interface/KeyboardNuclear.h"
...
{
public:
    void callback(const mobot6_interface::KeyboardNuclear::ConstPtr & message)
    {
        mCrtMsg = *message;
        mNewMsg = true;
    }
    ...
    const mobot6_interface::KeyboardNuclear& fetchMsg()
    {
        mNewMsg = false;
        return mCrtMsg;
    }
...
protected:
    mobot6_interface::KeyboardNuclear mCrtMsg;
    bool mNewMsg;
};
...
int main(int argc, char **argv)
{
    ...
    std::string COMMAND_ID[] = { COMMAND_ID_0, COMMAND_ID_1, COMMAND_ID_2, COMMAND_ID_3, COMMAND_ID_4,
                                 ...
                                 COMMAND_ID_10, COMMAND_ID_11, COMMAND_ID_12, COMMAND_ID_13 };  // new_added

    // create the input data and input state
    mobot6_interface::KeyboardNuclear   data_input;
    ...
    state_input.userFrame   = LINK_NAME[7];
    ...
    state_input.subKeyboardInput =
            Node_Handle.subscribe<mobot6_interface::KeyboardNuclear>(
    ...
    while (ros::ok())
    {
        try
        {
            ...
            if (COMMAND_HANDLLER.newMsg())
            {
	            ...
	            case 12:
                    ...
                    case 5:
                    {
                        ...
                        else if ((fabs(data_input.swings[0])!=2.0)||(fabs(data_input.swings[1])!=2.0))
                        {
                            state_input.swingControl.commandID = SWING_CMD_MOVE;
                            for (int i = 0; i < SWING_DOF; i++)
                    ...
                    case 13:
                    {
                        ...
                        else if ((data_input.nuclear[0]>=0.0) && (data_input.nuclear[1]>0.0))
                        {
                            state_input.nuclearControl.commandID = STP_CMD_MOVE;
                            for (int i = 0; i < NUCLEAR_DOF; i++)
                                state_input.nuclearControl.data[i] = data_input.nuclear[i];
                        }
                        else
                        {
                            state_input.nuclearControl.commandID = STP_CMD_OTHER;
            ...
            ROS_DEBUG("nuclearControl.commandID: %s.\n", state_input.nuclearControl.commandID.c_str());
